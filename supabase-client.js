...
const SupabaseService = {
  ...
  async fetchAllData() {
    ...
    try {
      ...
      const streets = (streetsRes.data || []).map(row => {
        let levels = ['B', 'A'];
        if (Array.isArray(row.levels) && row.levels.length > 0) {
          levels = row.levels;
        } else if (row.levels && typeof row.levels === 'object' && row.levels.list) {
          levels = row.levels.list;
        }
  ...
  async syncLocalToRemote(state) {
    ...
    if (state.streets && state.streets.length > 0) {
      const rows = state.streets.map(s => ({
        id: s.id,
        name: s.name,
        bays_count: s.slotsPerLevel || (s.baysCount ? s.baysCount * 2 : 8),
        levels: s.levels || ['B', 'A']
      }));
  ...
  async upsertStreet(street) {
    if (!this.client) return;
    try {
      const slotsPerLevel = street.slotsPerLevel || (street.baysCount ? street.baysCount * 2 : 8);
      await this.client.from('streets').upsert({
        id: street.id,
        name: street.name,
        bays_count: slotsPerLevel,
  ...
  async bulkUpsertCatalog(items) {
    ...
    try {
      const rows = items
        .filter(it => it && it.code && String(it.code).trim() !== '')
        .map(it => ({
          code: String(it.code).trim(),
          description: String(it.description || it.code).trim(),
          updated_at: new Date().toISOString()
        }));

      if (rows.length === 0) return;

      // Enviar em blocos de 100
      for (let i = 0; i < rows.length; i += 100) {
        const chunk = rows.slice(i, i + 100);
        const { error } = await this.client.from('catalog').upsert(chunk, { onConflict: 'code' });
        if (error) {
          console.warn('Erro ao inserir lote no catálogo Supabase:', error);
        }
      }
    } catch (err) {
