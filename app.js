...
// =============================================================================
// MÓDULO DE INVENTÁRIO FÍSICO: NOSSO SISTEMA vs TOTVS PROTHEUS
// =============================================================================
// SISTEMA DE INVENTÁRIO FÍSICO & RECONTAGENS: NOSSO SISTEMA vs TOTVS PROTHEUS
// =============================================================================

let inventorySession = {
  active: false,
  protheusFileName: '',
  round: 1, // 1: Confronto Inicial (1ª contagem), 2: 2ª contagem, 3: 3ª contagem...
  isFinalized: false,
...
function resetInventorySession() {
  if (inventorySession.active && !confirm('Deseja realmente reiniciar o inventário? Os dados da conferência atual serão descartados.')) {
    return;
  }
  inventorySession = {
    active: false,
  ...
  const pInput = document.getElementById('inventory-protheus-file-input');
  if (pInput) pInput.value = '';
  const rInput = document.getElementById('inventory-recount-file-input');
...
// -----------------------------------------------------------------------------
// Funções de Normalização para Correspondência Inteligente
// -----------------------------------------------------------------------------

function normalizeCode(val) {
  if (val === null || val === undefined) return '';
  let str = String(val).trim().toUpperCase();
  // Remove colchetes ou aspas: "[1040]" -> "1040"
...
function cleanAlphaNumeric(val) {
  if (val === null || val === undefined) return '';
  return String(val).toUpperCase().replace(/[^A-Z0-9]/g, '');
}

function normalizeLot(val) {
  if (val === null || val === undefined) return '';
  let str = String(val).trim().toUpperCase();
  if (str === '' || str === 'SEM LOTE' || str === '-' || str === '--' || str === 'N/A' ||
      str === '0' || str === 'PADRAO' || str === 'PADRÃO' || str === 'S/L' ||
      str === 'UNICO' || str === 'ÚNICO' || str === 'NAO SE APLICA' || str === 'NÃO SE APLICA') {
    return '';
  }
  return str.replace(/[\s\-_]/g, '');
}

function cleanLotForComparison(val) {
  let s = normalizeLot(val);
  if (!s) return '';
  // Remove prefixos comuns no Protheus: "LOTE", "LOT", "LT"
  s = s.replace(/^(LOTE|LOT|LT)/i, '');
  // Remove zeros à esquerda: "00002401" -> "2401"
...
// Analisador numérico ultra-robusto (suporta formatos BR 1.250,50 ou US 1,250.50 ou 350 UN)
function parseInventoryNumber(val) {
  if (val === null || val === undefined || val === '') return null;
  if (typeof val === 'number') return isNaN(val) ? 0 : val;
  let str = String(val).trim();
  if (!str) return null;

  // Remove sufixos de unidade de medida comuns
...
// -----------------------------------------------------------------------------
// Detecção Inteligente de Colunas e Abas na Planilha do TOTVS Protheus
// -----------------------------------------------------------------------------

function findBestSheet(workbook) {
  if (!workbook || !workbook.SheetNames || workbook.SheetNames.length === 0) return null;
  if (workbook.SheetNames.length === 1) return workbook.Sheets[workbook.SheetNames[0]];
  ...
  for (let sName of workbook.SheetNames) {
    ...
    // Se a aba tem cabeçalho de produto ou saldo, ganha bônus de pontuação
    for (let r = 0; r < Math.min(5, rows.length); r++) {
      const lineStr = (rows[r] || []).join(' ').toLowerCase();
      if (lineStr.includes('codigo') || lineStr.includes('produto') || lineStr.includes('b1_cod')) score += 1000;
      if (lineStr.includes('saldo') || lineStr.includes('quantidade') || lineStr.includes('b2_qatu')) score += 1000;
    }
  ...
  return bestSheet || workbook.Sheets[workbook.SheetNames[0]];
}

function detectProtheusColumns(rows) {
  let headerRowIndex = -1;
  let colCode = -1;
  let colDesc = -1;
  let colLot = -1;
  ...
  for (let r = 0; r < maxScan; r++) {
    ...
    let cCode = -1, cDesc = -1, cLot = -1, cQty = -1;

    for (let c = 0; c < row.length; c++) {
      const cell = normalizeText(row[c]);
      if (!cell) continue;

      // 1. Quantidade / Saldo (Prioridade Máxima)
      if (cQty === -1) {
        const isNotQty = cell.includes('empenh') || cell.includes('reserv') || cell.includes('bloquea') ||
                         cell.includes('anterior') || cell.includes('previst') || cell.includes('futur') ||
                         cell.includes('pedid') || cell.includes('minimo') || cell.includes('maximo') ||
                         cell.includes('seguranc') || cell.includes('preco') || cell.includes('custo') ||
                         cell.includes('valor');

        if (!isNotQty) {
          if (cell === 'quantidade do estoque' || cell === 'saldo atual' || cell === 'saldo fisico' ||
              cell === 'saldo em estoque' || cell === 'b2_qatu' || cell === 'qtd estoque' ||
              cell === 'qtde estoque' || cell === 'qtd. estoque' || cell === 'saldo disponivel' ||
              cell === 'quantidade' || cell === 'saldo' || cell === 'qtd' || cell === 'qtde' ||
              cell === 'estoque' || cell === 'quant' || cell.startsWith('saldo') || cell.startsWith('qtd')) {
            cQty = c;
            continue;
          }
        }
      ...
      // 2. Lote (Prioridade Alta)
      if (cLot === -1) {
        const isNotLot = cell.includes('economico') || cell.includes('minimo') || cell.includes('compra') || cell.includes('maximo');
        if (!isNotLot) {
          if (cell === 'lote' || cell === 'lotes' || cell === 'sublote' || cell === 'sub-lote' ||
              cell === 'b8_lotectl' || cell === 'd2_lotectl' || cell === 'num. lote' ||
              cell === 'num lote' || cell === 'lote controle' || cell.includes('lote')) {
            cLot = c;
            continue;
          }
        }
      ...
      // 3. Código do Produto
      if (cCode === -1) {
        const isNotCode = cell.includes('filial') || cell.includes('armazem') || cell.includes('local') ||
                          cell.includes('fornecedor') || cell.includes('cliente') || cell.includes('grupo') ||
                          cell.includes('tipo') || cell.includes('categoria') || cell.includes('unidade');
        if (!isNotCode) {
          if (cell === 'codigo' || cell === 'cod' || cell === 'cod.' || cell === 'produto' ||
              cell === 'cod produto' || cell === 'cod. produto' || cell === 'codigo produto' ||
              cell === 'codigo do produto' || cell === 'b1_cod' || cell === 'referencia' ||
              cell === 'ref' || cell === 'item' || cell === 'cod item') {
            cCode = c;
            continue;
          }
        }
      ...
      if (cDesc === -1) {
        const isNotDesc = cell.includes('filial') || cell.includes('armazem') || cell.includes('grupo') || cell.includes('tipo');
        if (!isNotDesc) {
          if (cell === 'descricao' || cell === 'desc' || cell === 'desc.' || cell === 'descricao do produto' ||
              cell === 'descricao produto' || cell === 'narrativa' || cell === 'b1_desc' ||
              cell === 'nome' || cell === 'nome produto' || cell.startsWith('desc')) {
            cDesc = c;
            continue;
          }
        }
    ...
    // Se encontrou ao menos código ou descrição E quantidade (ou código e descrição)
    if ((cCode !== -1 || cDesc !== -1) && (cQty !== -1 || (cCode !== -1 && cDesc !== -1))) {
      headerRowIndex = r;
      colCode = cCode;
      colDesc = cDesc;
      colLot = cLot;
  ...
  if (headerRowIndex === -1) {
    // Sem cabeçalho detectado: verifica se a linha 0 já é dado
    dataStartIndex = 0;
    if (rows.length > 0) {
      const firstCell = String(rows[0][0] || '').toLowerCase();
      if (firstCell.includes('totvs') || firstCell.includes('relat') || firstCell.includes('posicao') || firstCell.includes('protheus')) {
        dataStartIndex = 1;
      }
    }
  } else {
  ...
  // Fallbacks de coluna ordenados e sem colisão
  if (colCode === -1) colCode = 0;
  if (colDesc === -1) colDesc = colCode === 0 ? 1 : 0;
  
  if (colQty === -1) {
    // Tenta encontrar a primeira coluna com número válido
    const sampleRow = rows[dataStartIndex] || rows[0] || [];
    for (let c = 0; c < sampleRow.length; c++) {
      if (c !== colCode && c !== colDesc && c !== colLot) {
        const val = parseInventoryNumber(sampleRow[c]);
        if (val !== null) {
          colQty = c;
          break;
        }
  ...
  // Para Lote: só assume padrão (índice 2) se houver pelo menos 4 colunas e não colidir com Qty
  if (colLot === -1) {
    const sampleRow = rows[dataStartIndex] || rows[0] || [];
    if (sampleRow.length >= 4 && colQty !== 2 && colCode !== 2 && colDesc !== 2) {
      colLot = 2;
    } else {
      colLot = -1; // Sem coluna de lote na planilha
    }
  }

  // Garantia absoluta de que Lote e Qtd nunca apontem para a mesma coluna!
  if (colLot !== -1 && colLot === colQty) {
    colLot = -1;
  }
...
// -----------------------------------------------------------------------------
// Extração e Agrupamento do Estoque Físico do Almoxarifado Pro
// -----------------------------------------------------------------------------

function getOurWarehouseStockList() {
  const list = [];
  const slotsObj = appState.slots || {};

  // Varre 100% de todas as vagas salvas no sistema
  Object.keys(slotsObj).forEach(key => {
    const slot = slotsObj[key];
    if (!slot || !slot.items || slot.items.length === 0) return;

    const street = (appState.streets || []).find(s => s.id === slot.streetId) || { name: slot.streetId || 'Almoxarifado' };
    const address = `${street.name || 'Rua'} (${slot.code || key})`;

    slot.items.forEach(it => {
      const rawName = (it.productName || '').trim();
      const rawLot = (it.lot || '').trim();
      const qty = Number(it.quantity) || 0;
      if (!rawName && qty === 0) return;

      const parsed = extractProductCodeAndDesc(rawName);
      let code = parsed.code || '';
      let desc = parsed.desc || rawName;

      // Recuperação do código no catálogo (19.300+ itens) caso o produto tenha sido cadastrado apenas com a descrição
      if ((!code || code === desc) && (appState.catalog || window.PRELOADED_CATALOG)) {
        const cat = (appState.catalog && appState.catalog.length > 0) ? appState.catalog : (window.PRELOADED_CATALOG || []);
        const rawNorm = normalizeText(rawName);
        const catItem = cat.find(c =>
          c && (normalizeText(c.description) === rawNorm || normalizeCode(c.code) === normalizeCode(rawName))
        );
        if (catItem) {
          code = catItem.code;
          desc = catItem.description;
        }
      }

      list.push({
        streetId: slot.streetId,
        streetName: street.name,
        slotCode: slot.code || key,
        address: address,
        rawName: rawName,
        code: code,
        desc: desc,
...
function getAggregatedWarehouseStock() {
  const items = getOurWarehouseStockList();
  const groups = [];

  items.forEach(it => {
    const itNormCode = normalizeCode(it.code);
    const itCleanLot = cleanLotForComparison(it.lot);

    let found = groups.find(g => {
      const sameCode = (g.code && it.code && normalizeCode(g.code) === itNormCode) ||
                       (stripLeadingZeros(g.code) && stripLeadingZeros(it.code) && stripLeadingZeros(g.code) === stripLeadingZeros(it.code)) ||
                       (cleanAlphaNumeric(g.code) && cleanAlphaNumeric(it.code) && cleanAlphaNumeric(g.code) === cleanAlphaNumeric(it.code)) ||
                       (normalizeText(g.desc) === normalizeText(it.desc));
      const sameLot = cleanLotForComparison(g.lot) === itCleanLot;
      return sameCode && sameLot;
    });

    if (found) {
      found.quantity += it.quantity;
      if (!found.addresses.includes(it.address)) {
        found.addresses.push(it.address);
      }
    } else {
      groups.push({
        code: it.code,
...
// -----------------------------------------------------------------------------
// Motor de Correspondência Multi-Camadas (TOTVS Protheus vs Almoxarifado Pro)
// -----------------------------------------------------------------------------

function findWarehouseMatch(groups, rowCode, rowDesc, rowLot) {
  const normRowCode = normalizeCode(rowCode);
  const strippedRowCode = stripLeadingZeros(rowCode);
  const cleanAlphaRowCode = cleanAlphaNumeric(rowCode);
  const normRowDesc = normalizeText(rowDesc);
  const cleanRowLot = cleanLotForComparison(rowLot);

  // 1. Código Exato + Lote Limpo (sem prefixo LT/LOTE e sem zeros à esquerda)
  let match = groups.find(g =>
    !g.matched &&
    normRowCode && normalizeCode(g.code) === normRowCode &&
    cleanLotForComparison(g.lot) === cleanRowLot
  );
  if (match) return match;

  // 2. Código sem zeros à esquerda (ex: 0000001040 vs 1040) + Lote Limpo
  if (strippedRowCode) {
    match = groups.find(g =>
      !g.matched &&
      stripLeadingZeros(g.code) === strippedRowCode &&
      cleanLotForComparison(g.lot) === cleanRowLot
    );
    if (match) return match;
  }

  // 3. Código Alfanumérico Limpo (ignora pontos/traços: 01.02 vs 0102) + Lote Limpo
  if (cleanAlphaRowCode && cleanAlphaRowCode.length >= 3) {
    match = groups.find(g =>
      !g.matched &&
      cleanAlphaNumeric(g.code) === cleanAlphaRowCode &&
      cleanLotForComparison(g.lot) === cleanRowLot
    );
    if (match) return match;
  }

  // 4. Descrição Exata + Lote Limpo
  if (normRowDesc) {
    match = groups.find(g =>
      !g.matched &&
      (normalizeText(g.desc) === normRowDesc || normalizeText(g.rawName) === normRowDesc) &&
      cleanLotForComparison(g.lot) === cleanRowLot
    );
    if (match) return match;
  }

  // 5. Código bate (exato ou sem zeros), mas UM DOS LADOS NÃO TEM LOTE (ou lote não informado)
  if (!cleanRowLot) {
    const codeMatches = groups.filter(g =>
      !g.matched &&
      (
        (normRowCode && normalizeCode(g.code) === normRowCode) ||
        (strippedRowCode && stripLeadingZeros(g.code) === strippedRowCode) ||
        (cleanAlphaRowCode && cleanAlphaNumeric(g.code) === cleanAlphaRowCode)
      )
    );
    if (codeMatches.length > 0) return codeMatches;
  }

  // 5b. Protheus tem lote, mas no Almoxarifado o produto foi registrado sem lote
  if (cleanRowLot) {
    const noLotMatches = groups.filter(g =>
      !g.matched &&
      !cleanLotForComparison(g.lot) &&
      (
        (normRowCode && normalizeCode(g.code) === normRowCode) ||
        (strippedRowCode && stripLeadingZeros(g.code) === strippedRowCode) ||
        (cleanAlphaRowCode && cleanAlphaNumeric(g.code) === cleanAlphaRowCode)
    ...
    if (noLotMatches.length > 0) return noLotMatches;
  }

  // 6. Descrição contém parte substancial (>= 5 letras) + Lote Limpo
  if (normRowDesc && normRowDesc.length >= 5) {
    match = groups.find(g =>
      !g.matched &&
      (normalizeText(g.desc).includes(normRowDesc) || normRowDesc.includes(normalizeText(g.desc))) &&
      cleanLotForComparison(g.lot) === cleanRowLot
    );
    if (match) return match;
  }

  // 7. Código contido no nome original (ex: "[1040] Parafuso...") + Lote Limpo
  if (normRowCode && normRowCode.length >= 3) {
    match = groups.find(g =>
      !g.matched &&
      normalizeText(g.rawName).includes(normRowCode.toLowerCase()) &&
      (cleanLotForComparison(g.lot) === cleanRowLot || !cleanRowLot || !cleanLotForComparison(g.lot))
    );
    if (match) return match;
  }

  // 8. Se ainda não achou e a descrição bate exatamente sem considerar lote
  if (normRowDesc) {
    const descMatches = groups.filter(g =>
      !g.matched &&
      (normalizeText(g.desc) === normRowDesc || normalizeText(g.rawName) === normRowDesc)
    );
...
// -----------------------------------------------------------------------------
// Baixar Modelo do TOTVS Protheus (4 Colunas)
// -----------------------------------------------------------------------------

function downloadProtheusTemplate() {
  const header = ['Codigo', 'Descricao', 'Lotes', 'Quantidade do estoque'];
  ...
  if (consolidated.length > 0) {
    ...
    rows = [
      ['1040', 'Parafuso Sextavado M8x25', 'LOTE-2401', 350],
  ...
  if (typeof XLSX !== 'undefined') {
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.aoa_to_sheet(aoa);
    ws['!cols'] = [{ wch: 20 }, { wch: 45 }, { wch: 20 }, { wch: 25 }];
    XLSX.utils.book_append_sheet(wb, ws, 'PROTHEUS_ESTOQUE');
    const today = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(wb, `Modelo_TOTVS_Protheus_${today}.xlsx`);
  } else {
    alert('Biblioteca de planilhas indisponível no navegador.');
  }
}

// -----------------------------------------------------------------------------
// 1. Processamento e Confronto: Nosso Sistema vs Planilha TOTVS PROTHEUS
// -----------------------------------------------------------------------------

function handleProtheusFileUpload(file) {
  if (!file) return;
  if (typeof XLSX === 'undefined') {
    alert('Erro: Biblioteca XLSX não disponível.');
    return;
  }
  ...
  reader.onload = (e) => {
    try {
      ...
      const worksheet = findBestSheet(workbook);
      if (!worksheet) {
        alert('A planilha selecionada está vazia ou ilegível.');
        return;
      }
      ...
      if (!rows || rows.length === 0) {
        alert('A planilha não contém linhas de dados.');
        return;
      }

      processProtheusComparison(rows, file.name);
    } catch (err) {
      console.error('Erro ao ler planilha TOTVS Protheus:', err);
...
function processProtheusComparison(rows, fileName) {
  const { dataStartIndex, colCode, colDesc, colLot, colQty } = detectProtheusColumns(rows);
  ...
  const warehouseGroups = getAggregatedWarehouseStock();
  const analyzedItems = [];

  dataRows.forEach(r => {
    if (!r || !Array.isArray(r)) return;
    const rawCode = colCode !== -1 ? String(r[colCode] || '').trim() : '';
    const rawDesc = colDesc !== -1 ? String(r[colDesc] || '').trim() : '';
    const rawLot = colLot !== -1 ? String(r[colLot] || '').trim() : '';
    const protheusQty = colQty !== -1 ? (parseInventoryNumber(r[colQty]) ?? 0) : 0;

    if (!rawCode && !rawDesc) return;

    const matchResult = findWarehouseMatch(warehouseGroups, rawCode, rawDesc, rawLot);

    let systemQty = 0;
    let locations = '(Não consta nas vagas)';
    let isFoundInSystem = false;
    let displayCode = rawCode;
    let displayDesc = rawDesc;
    let displayLot = rawLot;

    if (Array.isArray(matchResult)) {
      isFoundInSystem = true;
      systemQty = matchResult.reduce((sum, m) => sum + m.quantity, 0);
      locations = matchResult.flatMap(m => m.addresses).join(', ');
      matchResult.forEach(m => { m.matched = true; });
      if (matchResult[0]) {
        displayCode = rawCode || matchResult[0].code;
        displayDesc = rawDesc || matchResult[0].desc;
        if (!displayLot) displayLot = matchResult[0].lot;
      }
    } else if (matchResult) {
      isFoundInSystem = true;
      systemQty = matchResult.quantity;
      locations = matchResult.addresses.length > 0 ? matchResult.addresses.join(', ') : '(Sem endereço)';
      matchResult.matched = true;
      displayCode = rawCode || matchResult.code;
      displayDesc = rawDesc || matchResult.desc;
      displayLot = rawLot || matchResult.lot;
    }

    const c1 = systemQty;
    const isExactMatch = isFoundInSystem && Math.abs(c1 - protheusQty) < 0.0001;

    let status = 'APPROVED';
    let statusLabel = '🟢 100% Bateu';
    ...
    if (isExactMatch) {
      status = 'APPROVED';
      statusLabel = '🟢 100% Bateu';
      statusClass = 'row-approved';
      needsRecount = false;
      approvedQty = c1;
    } else if (isFoundInSystem && c1 === 0 && protheusQty === 0) {
      status = 'APPROVED';
      statusLabel = '🟢 100% Bateu (Zerado)';
      statusClass = 'row-approved';
      needsRecount = false;
      approvedQty = 0;
    } else if (!isFoundInSystem) {
      status = 'DIVERGENT';
      statusLabel = '🔴 Falta no Almoxarifado';
      statusClass = 'row-divergent-2';
      needsRecount = true;
    } else {
      status = 'DIVERGENT';
      statusLabel = c1 > protheusQty ? '🔴 Sobra no Sistema (+)' : '🔴 Falta no Sistema (-)';
    ...
    analyzedItems.push({
      ...
      c3: null,
      approvedQty,
      diff: (approvedQty !== null ? approvedQty : c1) - protheusQty,
      status,
      statusLabel,
      statusClass,
      needsRecount,
  ...
  // Sobras Físicas: materiais presentes nas vagas que não vieram na planilha do Protheus
  warehouseGroups.forEach(g => {
    if (!g.matched && g.quantity > 0) {
      analyzedItems.push({
        code: g.code,
        description: g.desc,
        lot: g.lot,
        ...
        diff: g.quantity,
        status: 'DIVERGENT',
        statusLabel: '⚠️ Sobra Física (Não Consta no Protheus)',
        statusClass: 'row-divergent-3',
        needsRecount: true,
        locations: g.addresses.length > 0 ? g.addresses.join(', ') : '(Sem endereço)'
      });
    }
  });

  const colLetter = (idx) => idx !== -1 ? String.fromCharCode(65 + idx) : 'N/A';

  inventorySession.active = true;
  inventorySession.round = 1;
  inventorySession.isFinalized = false;
  inventorySession.protheusFileName = fileName;
  inventorySession.detectedColumns = {
    colCode: colLetter(colCode),
    colDesc: colLetter(colDesc),
    colLot: colLetter(colLot),
    colQty: colLetter(colQty),
    headerRow: dataStartIndex
  };
  inventorySession.items = analyzedItems;

  renderInventoryUI();
}

// -----------------------------------------------------------------------------
// 2. Exportar Planilha de Recontagem (somente itens divergentes)
// -----------------------------------------------------------------------------

function downloadRecountExcel() {
  const recountItems = inventorySession.items.filter(it => it.needsRecount);
  if (recountItems.length === 0) {
    alert('Parabéns! Não existem divergências para recontar. Todos os materiais bateram perfeitamente com o TOTVS Protheus!');
    return;
  }

  const round = inventorySession.round;
  let header = [];
  let rows = [];

  if (round === 1) {
    // 2ª Contagem: Colunas A, B, C, D (1ª contagem preenchida), E (2ª contagem em branco)
    header = ['Codigo', 'Descricao', 'Lotes', 'Primeira contagem', 'Segunda contagem'];
    rows = recountItems.map(it => [
      it.code,
      it.description,
      it.lot || '',
      it.c1 !== null && it.c1 !== undefined ? it.c1 : 0,
      ''
    ]);
  } else if (round === 2) {
    // 3ª Contagem: Colunas A, B, C, D, E preenchidas, F (3ª contagem em branco)
    header = ['Codigo', 'Descricao', 'Lotes', 'Primeira contagem', 'Segunda contagem', 'Terceira contagem'];
    rows = recountItems.map(it => [
      it.code,
      it.description,
      it.lot || '',
      it.c1 !== null && it.c1 !== undefined ? it.c1 : 0,
      it.c2 !== null && it.c2 !== undefined ? it.c2 : '',
      ''
    ]);
  } else {
    header = ['Codigo', 'Descricao', 'Lotes', 'Primeira contagem', 'Segunda contagem', 'Terceira contagem', 'Quarta contagem'];
    rows = recountItems.map(it => [
      it.code,
      it.description,
      it.lot || '',
      it.c1 !== null && it.c1 !== undefined ? it.c1 : 0,
      it.c2 !== null && it.c2 !== undefined ? it.c2 : '',
      it.c3 !== null && it.c3 !== undefined ? it.c3 : '',
      ''
    ]);
  }
  ...
  if (typeof XLSX !== 'undefined') {
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.aoa_to_sheet(aoa);
    ws['!cols'] = [{ wch: 20 }, { wch: 45 }, { wch: 18 }, { wch: 22 }, { wch: 22 }, { wch: 22 }];
    const phaseName = round === 1 ? '2a_Contagem' : (round === 2 ? '3a_Contagem' : `${round + 1}a_Contagem`);
    XLSX.utils.book_append_sheet(wb, ws, `RECONTAGEM_${phaseName}`);
    const today = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(wb, `Planilha_Recontagem_${phaseName}_${today}.xlsx`);
  } else {
...
// -----------------------------------------------------------------------------
// 3. Processar Envio da Recontagem (Segunda ou Terceira Contagem)
// -----------------------------------------------------------------------------

function handleRecountFileUpload(file) {
  if (!file) return;
  if (!inventorySession.active || inventorySession.items.length === 0) {
    alert('Por favor, primeiro importe a planilha do TOTVS Protheus.');
    return;
  }
  ...
  reader.onload = (e) => {
    try {
      ...
      if (!worksheet) {
        alert('A planilha de recontagem está vazia.');
        return;
      }

      const rows = XLSX.utils.sheet_to_json(worksheet, { header: 1, defval: '' });
      if (!rows || rows.length <= 1) {
        alert('A planilha de recontagem não possui linhas preenchidas.');
        return;
      }

      processRecountData(rows);
    } catch (err) {
      console.error(err);
      alert('Erro ao ler planilha de recontagem: ' + err.message);
    }
  };
  reader.readAsArrayBuffer(file);
}

function processRecountData(rows) {
  const headerRow = rows[0] || [];
  const dataRows = rows.slice(1);
  const currentRound = inventorySession.round;
  let recheckedCount = 0;

  // Auto-detecta a coluna onde está a contagem
  let targetCol = -1;
  const targetLabel = currentRound === 1 ? 'segunda' : 'terceira';

  for (let c = 0; c < headerRow.length; c++) {
    const h = normalizeText(headerRow[c]);
    if (h.includes(targetLabel) && (h.includes('contagem') || h.includes('qtd') || h.includes('saldo'))) {
      targetCol = c;
      break;
    }
  }
  ...
  if (targetCol === -1) {
    targetCol = currentRound === 1 ? 4 : 5; // Coluna E (índice 4) ou Coluna F (índice 5)
  }

  dataRows.forEach(r => {
    const rawCode = String(r[0] || '').trim();
    const rawDesc = String(r[1] || '').trim();
    const rawLot = String(r[2] || '').trim();
    if (!rawCode && !rawDesc) return;

    const normCode = normalizeCode(rawCode);
    const strippedCode = stripLeadingZeros(rawCode);
    const cleanAlpha = cleanAlphaNumeric(rawCode);
    const cleanLot = cleanLotForComparison(rawLot);
    const normDesc = normalizeText(rawDesc);

    // Localiza o item no inventário de forma tolerante e precisa
    const item = inventorySession.items.find(it => {
      const matchCode = (it.code && normalizeCode(it.code) === normCode) ||
                        (stripLeadingZeros(it.code) && stripLeadingZeros(it.code) === strippedCode) ||
                        (cleanAlphaNumeric(it.code) && cleanAlphaNumeric(it.code) === cleanAlpha);
      const matchLot = cleanLotForComparison(it.lot) === cleanLot;
      return matchCode && matchLot;
    }) || inventorySession.items.find(it => {
      const matchCode = (it.code && normalizeCode(it.code) === normCode) ||
                        (stripLeadingZeros(it.code) && stripLeadingZeros(it.code) === strippedCode);
      return matchCode;
    }) || inventorySession.items.find(it => {
      return normDesc && normalizeText(it.description) === normDesc;
    });

    if (!item) return;

    if (currentRound === 1) {
      const c2 = parseInventoryNumber(r[targetCol]);
      if (c2 !== null) {
        item.c2 = c2;
        recheckedCount++;

        // REGRA OFICIAL DE INVENTÁRIO:
        // Se bater com o TOTVS Protheus OU com a 1ª contagem -> APROVADO!
        if (Math.abs(item.c2 - item.protheusQty) < 0.0001) {
          item.status = 'APPROVED';
          item.statusLabel = '🟢 Aprovado (Bateu com Protheus)';
          item.statusClass = 'row-approved';
          item.needsRecount = false;
          item.approvedQty = item.c2;
        } else if (Math.abs(item.c2 - item.c1) < 0.0001) {
          item.status = 'APPROVED';
          item.statusLabel = '🟢 Aprovado (Bateu com 1ª Contagem)';
          item.statusClass = 'row-approved';
          item.needsRecount = false;
          item.approvedQty = item.c2;
        } else {
          item.status = 'DIVERGENT';
          item.statusLabel = '🔴 Divergente na 2ª Contagem';
          item.statusClass = 'row-divergent-2';
          item.needsRecount = true;
          item.approvedQty = null;
        }
      }
    } else if (currentRound === 2) {
      const c3 = parseInventoryNumber(r[targetCol]);
      if (c3 !== null) {
        item.c3 = c3;
        recheckedCount++;

        if (Math.abs(item.c3 - item.protheusQty) < 0.0001 ||
            (item.c2 !== null && Math.abs(item.c3 - item.c2) < 0.0001) ||
            Math.abs(item.c3 - item.c1) < 0.0001) {
          item.status = 'APPROVED';
          item.statusLabel = '🟢 Aprovado na 3ª Contagem';
          item.statusClass = 'row-approved';
          item.needsRecount = false;
          item.approvedQty = item.c3;
        } else {
          item.status = 'DIVERGENT';
          item.statusLabel = '⚠️ Divergente na 3ª Contagem';
          item.statusClass = 'row-divergent-3';
          item.needsRecount = true;
          item.approvedQty = null;
        }
      }
    ...
    const currentFinal = item.approvedQty !== null ? item.approvedQty : (item.c3 ?? item.c2 ?? item.c1 ?? 0);
    item.diff = currentFinal - item.protheusQty;
  });

  if (recheckedCount > 0) {
    inventorySession.round = currentRound + 1;
  }

  renderInventoryUI();

  const remainingDivergent = inventorySession.items.filter(it => it.needsRecount).length;
  if (remainingDivergent === 0) {
    alert(`Excelente! Todas as ${recheckedCount} recontagens foram processadas e 100% dos materiais estão aprovados!`);
  } else {
    alert(`Recontagem processada: ${recheckedCount} materiais verificados. Ainda restam ${remainingDivergent} divergências.`);
  }
}

// -----------------------------------------------------------------------------
// 4. Finalizar Inventário & Baixar Planilha Consolidada Completa
// -----------------------------------------------------------------------------

function finalizeInventory() {
  if (!inventorySession.items || inventorySession.items.length === 0) {
    alert('Nenhum dado de inventário disponível para finalizar.');
    return;
  }

  const divergentRemaining = inventorySession.items.filter(it => it.needsRecount).length;
  let confirmMsg = 'Deseja finalizar esta operação de inventário?\n\nSerá gerada e baixada a planilha final consolidada com a QUANTIDADE DO TOTVS PROTHEUS e TODAS as contagens realizadas.';
  if (divergentRemaining > 0) {
    confirmMsg += `\n\n⚠️ Atenção: Ainda existem ${divergentRemaining} materiais divergentes que serão consolidados com a última contagem feita.`;
  }

  if (!confirm(confirmMsg)) return;

  inventorySession.isFinalized = true;

  inventorySession.items.forEach(it => {
    it.needsRecount = false;
    if (it.approvedQty === null || it.approvedQty === undefined) {
      it.approvedQty = it.c3 ?? it.c2 ?? it.c1 ?? 0;
    }
    it.diff = it.approvedQty - it.protheusQty;
    if (it.status === 'DIVERGENT') {
      it.statusLabel = '🏁 Finalizado com Auditoria';
      it.statusClass = 'row-divergent-final';
    }
  });

  const hasC2 = inventorySession.items.some(it => it.c2 !== null && it.c2 !== undefined);
  const hasC3 = inventorySession.items.some(it => it.c3 !== null && it.c3 !== undefined);

  const header = [
    'Codigo',
    'Descricao',
    'Lotes',
    'Quantidade_Estoque_Protheus',
    'Primeira_Contagem'
  ];

  if (hasC2) header.push('Segunda_Contagem');
  if (hasC3) header.push('Terceira_Contagem');

  header.push(
    'Saldo_Final_Aprovado',
    'Diferenca_Final',
    'Status_Final',
    'Posicoes_Almoxarifado'
  );

  const rows = inventorySession.items.map(it => {
    const row = [
      it.code,
      it.description,
      it.lot || 'SEM LOTE',
      it.protheusQty,
      it.c1 !== null && it.c1 !== undefined ? it.c1 : '-'
    ];

    if (hasC2) row.push(it.c2 !== null && it.c2 !== undefined ? it.c2 : '-');
    if (hasC3) row.push(it.c3 !== null && it.c3 !== undefined ? it.c3 : '-');
  ...
  if (typeof XLSX !== 'undefined') {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'INVENTARIO_CONSOLIDADO');
    const today = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(wb, `Planilha_Final_Inventario_TOTVS_${today}.xlsx`);
  }

  renderInventoryUI();
  alert('Inventário finalizado com sucesso! A planilha consolidada contendo o saldo TOTVS Protheus e todas as contagens foi baixada.');
}

// -----------------------------------------------------------------------------
// 5. Atualização Opcional das Vagas Físicas com o Saldo Final Aprovado
// -----------------------------------------------------------------------------

function applyInventoryAdjustments() {
  if (!inventorySession.isFinalized) {
    alert('Por favor, primeiro finalize o inventário para consolidar os saldos.');
    return;
  }

  const divergent = inventorySession.items.filter(it => Math.abs(it.diff) > 0.0001);
  if (divergent.length === 0) {
    alert('Todos os saldos físicos já estão 100% idênticos aos aprovados!');
    return;
  }

  if (!confirm(`Deseja atualizar as vagas do Almoxarifado Pro para refletirem exatamente os saldos finais auditados de ${divergent.length} materiais com diferenças?`)) {
    return;
  }

  let adjustedCount = 0;
  divergent.forEach(it => {
    const targetQty = it.approvedQty;
    // Localiza e ajusta nas vagas
    Object.keys(appState.slots || {}).forEach(k => {
      const slot = appState.slots[k];
      if (!slot || !slot.items) return;
      slot.items.forEach(slotIt => {
        const parsed = extractProductCodeAndDesc(slotIt.productName);
        const matchCode = (parsed.code && it.code && normalizeCode(parsed.code) === normalizeCode(it.code)) ||
                          (stripLeadingZeros(parsed.code) && stripLeadingZeros(it.code) && stripLeadingZeros(parsed.code) === stripLeadingZeros(it.code)) ||
                          (normalizeText(parsed.desc) === normalizeText(it.description));
        const matchLot = cleanLotForComparison(slotIt.lot) === cleanLotForComparison(it.lot);
        if (matchCode && matchLot) {
          slotIt.quantity = targetQty;
          adjustedCount++;
        }
      });
    });
  ...
  renderAll();
  renderInventoryUI();
  alert(`Sucesso: ${adjustedCount} registros de estoque foram atualizados para refletir o inventário auditado.`);
}

// -----------------------------------------------------------------------------
// 6. Renderização da Interface Visual do Inventário
// -----------------------------------------------------------------------------

function renderInventoryUI() {
  const uploadZone = document.getElementById('inventory-upload-zone');
  const accuracyCard = document.getElementById('inventory-accuracy-card');
  const kpisPanel = document.getElementById('inventory-kpis-panel');
  const controlsBar = document.getElementById('inventory-table-controls');
  const tableWrapper = document.getElementById('inventory-table-wrapper');
  const guidanceBox = document.getElementById('inventory-guidance-box');

  const btnTemplate = document.getElementById('btn-download-inv-template');
  const btnImport = document.getElementById('btn-import-protheus');
  const btnRecount = document.getElementById('btn-download-recount');
  const btnUploadRecount = document.getElementById('btn-upload-recount');
  const btnFinalize = document.getElementById('btn-finalize-inventory');
  const btnReset = document.getElementById('btn-reset-inventory');
  const btnApply = document.getElementById('btn-apply-inventory-adjust');

  const recountCountSpan = document.getElementById('recount-items-count');
  const footerText = document.getElementById('inventory-summary-footer-text');

  if (!inventorySession.active || inventorySession.items.length === 0) {
    if (uploadZone) uploadZone.classList.remove('hidden');
    if (accuracyCard) accuracyCard.classList.add('hidden');
    if (kpisPanel) kpisPanel.classList.add('hidden');
    if (controlsBar) controlsBar.classList.add('hidden');
    if (tableWrapper) tableWrapper.classList.add('hidden');
    if (guidanceBox) guidanceBox.classList.add('hidden');

    if (btnRecount) btnRecount.classList.add('hidden');
    if (btnUploadRecount) btnUploadRecount.classList.add('hidden');
    if (btnFinalize) btnFinalize.classList.add('hidden');
    if (btnReset) btnReset.classList.add('hidden');
    if (btnApply) btnApply.classList.add('hidden');

    if (footerText) footerText.textContent = 'Aguardando planilha do TOTVS Protheus...';
    return;
  }
  ...
  if (kpisPanel) kpisPanel.classList.remove('hidden');
  if (controlsBar) controlsBar.classList.remove('hidden');
  if (tableWrapper) tableWrapper.classList.remove('hidden');
  if (guidanceBox) guidanceBox.classList.remove('hidden');

  if (btnReset) btnReset.classList.remove('hidden');
  if (btnFinalize) btnFinalize.classList.remove('hidden');

  const total = inventorySession.items.length;
  const approved = inventorySession.items.filter(it => !it.needsRecount).length;
  const divergent = inventorySession.items.filter(it => it.needsRecount).length;
  const accuracyPct = total > 0 ? ((approved / total) * 100).toFixed(1) : '100.0';

  let totalDiffUnits = 0;
  let totalAuditUnits = 0;
  inventorySession.items.forEach(it => {
    totalDiffUnits += Math.abs(it.diff || 0);
    totalAuditUnits += (it.protheusQty || 0);
  });

  // Atualiza Acurácia
  const meterPct = document.getElementById('accuracy-meter-pct');
  const meterBadge = document.getElementById('accuracy-meter-badge');
  const meterFill = document.getElementById('accuracy-bar-fill');
  const summaryText = document.getElementById('accuracy-summary-text');
  const summaryUnits = document.getElementById('accuracy-summary-units');

  if (meterPct) meterPct.textContent = `${accuracyPct}%`;
  if (meterFill) {
    meterFill.style.width = `${accuracyPct}%`;
    meterFill.className = 'accuracy-bar-fill ' + (accuracyPct >= 98 ? 'fill-excellent' : (accuracyPct >= 85 ? 'fill-good' : 'fill-warning'));
  }

  if (meterBadge) {
    if (accuracyPct >= 98) {
      meterBadge.className = 'accuracy-badge accuracy-badge-excellent';
      meterBadge.textContent = '🟢 100% Acurado';
    } else if (accuracyPct >= 85) {
      meterBadge.className = 'accuracy-badge accuracy-badge-good';
      meterBadge.textContent = '🟡 Acurácia Razoável';
    } else {
      meterBadge.className = 'accuracy-badge accuracy-badge-poor';
      meterBadge.textContent = '🔴 Divergências Críticas';
    }
  }

  if (summaryText) {
    if (divergent === 0) {
      summaryText.textContent = 'Perfeito! 100% dos materiais conferem exatamente com o TOTVS Protheus.';
    } else {
      summaryText.textContent = `${approved} de ${total} materiais 100% conferidos. Restam ${divergent} divergências para recontagem.`;
    }
  }
  if (summaryUnits) {
    summaryUnits.textContent = `${totalAuditUnits.toLocaleString('pt-BR')} un TOTVS auditadas`;
  }

  // KPIs
  const elTotal = document.getElementById('inv-kpi-total');
  const elTotalUnits = document.getElementById('inv-kpi-total-units');
  const elApproved = document.getElementById('inv-kpi-approved');
  const elDivergent = document.getElementById('inv-kpi-divergent');
  const elDiffUnits = document.getElementById('inv-kpi-diff-units');
  const elPhaseLabel = document.getElementById('inv-kpi-phase-label');

  if (elTotal) elTotal.textContent = total.toLocaleString('pt-BR');
  if (elTotalUnits) elTotalUnits.textContent = `${totalAuditUnits.toLocaleString('pt-BR')} un no Protheus`;
  if (elApproved) elApproved.textContent = approved.toLocaleString('pt-BR');
  if (elDivergent) elDivergent.textContent = divergent.toLocaleString('pt-BR');
  if (elDiffUnits) elDiffUnits.textContent = `${totalDiffUnits.toLocaleString('pt-BR')} un`;
  if (elPhaseLabel) {
    elPhaseLabel.textContent = inventorySession.round === 1 ? 'Aguardando 2ª contagem' : `${inventorySession.round + 1}ª rodada`;
  }

  // Botões de Recontagem e Finalização
  if (btnRecount) {
    if (divergent > 0 && !inventorySession.isFinalized) {
      btnRecount.classList.remove('hidden');
      if (recountCountSpan) recountCountSpan.textContent = divergent;
      const roundLabel = inventorySession.round === 1 ? '2ª Contagem' : `${inventorySession.round + 1}ª Contagem`;
      btnRecount.innerHTML = `🔄 Baixar Planilha para Recontagem (${divergent})`;
    } else {
      btnRecount.classList.add('hidden');
    }
  }

  if (btnUploadRecount) {
    if (divergent > 0 && !inventorySession.isFinalized) {
      btnUploadRecount.classList.remove('hidden');
      const roundUploadLabel = inventorySession.round === 1 ? 'Enviar Segunda Contagem' : `Enviar ${inventorySession.round + 1}ª Contagem`;
      btnUploadRecount.innerHTML = `📤 ${roundUploadLabel}`;
    } else {
      btnUploadRecount.classList.add('hidden');
    }
  }
  ...
  // Caixa de Orientação Passo a Passo
  if (guidanceBox) {
    let colInfo = '';
    if (inventorySession.detectedColumns) {
      const d = inventorySession.detectedColumns;
      colInfo = `
        <div style="margin-top: 0.5rem; padding: 0.5rem 0.75rem; background: rgba(15, 23, 42, 0.6); border-radius: 6px; font-size: 0.8rem; border: 1px solid rgba(255,255,255,0.08);">
          🔍 <strong>Colunas Reconhecidas da sua Planilha:</strong> 
          Código: <strong style="color: #38bdf8;">Coluna ${d.colCode}</strong> | 
          Descrição: <strong style="color: #38bdf8;">Coluna ${d.colDesc}</strong> | 
          Lote: <strong style="color: #fbbf24;">Coluna ${d.colLot}</strong> | 
          Saldo TOTVS: <strong style="color: #10b981;">Coluna ${d.colQty}</strong>
        </div>
      `;
    }

    if (inventorySession.isFinalized) {
      guidanceBox.className = 'inventory-guidance-card guidance-success';
      guidanceBox.innerHTML = `
        <div class="guidance-title">🏁 Inventário Finalizado com Sucesso!</div>
        <p>A auditoria foi encerrada e a <strong>Planilha Final Consolidada</strong> foi baixada. Caso deseje que os saldos finais auditados sejam gravados diretamente nas vagas do Almoxarifado Pro, clique no botão <strong>"Atualizar Vagas do Almoxarifado"</strong> acima.</p>
        ${colInfo}
      `;
    } else if (divergent === 0) {
      guidanceBox.className = 'inventory-guidance-card guidance-success';
      guidanceBox.innerHTML = `
        <div class="guidance-title">🟢 100% de Acurácia! Estoque em Perfeita Sintonia</div>
        <p>Todos os ${total} materiais conferem perfeitamente entre o Almoxarifado Pro e o TOTVS Protheus. Você já pode clicar em <strong>"Finalizar Inventário"</strong> para emitir o relatório consolidado oficial.</p>
        ${colInfo}
      `;
    } else if (inventorySession.round === 1) {
      guidanceBox.className = 'inventory-guidance-card guidance-warning';
      guidanceBox.innerHTML = `
        <div class="guidance-title">📋 Passo 1: Planilha do TOTVS Protheus Processada!</div>
        <p>O sistema confrontou o estoque físico das vagas (1ª Contagem) com a planilha do TOTVS Protheus. 
           Foram encontrados <strong>${divergent} materiais divergentes</strong>.<br>
           👉 Clique em <strong>"Baixar Planilha para Recontagem"</strong>: o sistema gerará a planilha com a Coluna D (Primeira contagem) preenchida e a <strong>Coluna E (Segunda contagem) em branco</strong> para conferência física em campo. Ao terminar, clique em <strong>"Enviar Segunda Contagem"</strong>.</p>
        ${colInfo}
      `;
    } else {
      guidanceBox.className = 'inventory-guidance-card guidance-warning';
      guidanceBox.innerHTML = `
        <div class="guidance-title">🔄 Passo 2: Segunda Contagem Analisada (${divergent} restantes)</div>
        <p>A 2ª contagem foi processada. Pela regra de negócio, os materiais que bateram com o Protheus OU com a 1ª contagem foram aprovados. Ainda restam <strong>${divergent} divergências</strong>. Você pode baixar a planilha de <strong>3ª contagem</strong> ou <strong>"Finalizar Inventário"</strong> a qualquer momento.</p>
        ${colInfo}
      `;
    }
  }
  ...
  if (theadRow) {
    ...
    let ths = `
      ...
      <th>Lote</th>
      <th>Saldo TOTVS</th>
      <th>1ª Contagem (Sistema)</th>
    `;
    if (hasC2) ths += `<th>2ª Contagem</th>`;
    if (hasC3) ths += `<th>3ª Contagem</th>`;
    ths += `
      <th>Saldo Final</th>
      <th>Diferença</th>
      <th>Endereço Almoxarifado</th>
    `;
    theadRow.innerHTML = ths;
  }

  // Filtros e Linhas da Tabela
  const searchInput = document.getElementById('filter-inventory-search');
  ...
  let filtered = [...inventorySession.items];
  ...
  if (tbody) {
    tbody.innerHTML = '';
    if (filtered.length === 0) {
      tbody.innerHTML = `<tr><td colspan="11" class="text-muted" style="text-align: center; padding: 2rem;">Nenhum material encontrado com o filtro selecionado.</td></tr>`;
    } else {
      const hasC2 = inventorySession.items.some(it => it.c2 !== null && it.c2 !== undefined);
      const hasC3 = inventorySession.items.some(it => it.c3 !== null && it.c3 !== undefined);
      ...
      filtered.forEach(it => {
        ...
        if (it.status === 'DIVERGENT') {
          if (it.c2 !== null) badgeClass = 'badge-inv-recount2';
          else badgeClass = 'badge-inv-recount1';
        }
        ...
        let rowHtml = `
          ...
          <td>${escapeHtml(it.description)}</td>
          <td>${it.lot ? `<span class="inv-location-tag">Lt: ${escapeHtml(it.lot)}</span>` : '<span class="text-dim">Sem lote</span>'}</td>
          <td><strong style="color: #cbd5e1; font-family: 'JetBrains Mono', monospace;">${it.protheusQty.toLocaleString('pt-BR')} un</strong></td>
          <td><strong style="color: #38bdf8; font-family: 'JetBrains Mono', monospace;">${it.c1 !== null && it.c1 !== undefined ? `${it.c1.toLocaleString('pt-BR')} un` : '-'}</strong></td>
        `;

        if (hasC2) {
          rowHtml += `<td>${it.c2 !== null && it.c2 !== undefined ? `<strong style="color: #fbbf24; font-family: 'JetBrains Mono', monospace;">${it.c2.toLocaleString('pt-BR')} un</strong>` : '<span class="text-dim">-</span>'}</td>`;
        }
        if (hasC3) {
          rowHtml += `<td>${it.c3 !== null && it.c3 !== undefined ? `<strong style="color: #c084fc; font-family: 'JetBrains Mono', monospace;">${it.c3.toLocaleString('pt-BR')} un</strong>` : '<span class="text-dim">-</span>'}</td>`;
        }

        const finalShow = it.approvedQty !== null ? it.approvedQty : (it.c3 ?? it.c2 ?? it.c1 ?? 0);
        rowHtml += `
          <td><strong style="color: #10b981; font-family: 'JetBrains Mono', monospace;">${finalShow.toLocaleString('pt-BR')} un</strong></td>
  ...
  if (footerText) {
    footerText.textContent = `Arquivo: ${inventorySession.protheusFileName || 'TOTVS Protheus'} • ${total} materiais auditados (${approved} aprovados, ${divergent} divergências) • Acurácia: ${accuracyPct}%`;
  }
}

// -----------------------------------------------------------------------------
// Drag & Drop na Dropzone de Inventário
// -----------------------------------------------------------------------------
...
function setupInventoryDragAndDrop() {
  ...
  dropzone.addEventListener('drop', (e) => {
    ...
    if (dt && dt.files && dt.files[0]) {
      handleProtheusFileUpload(dt.files[0]);
    }
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('filter-stock-grouping').addEventListener('change', renderStockTable);
    ...
    // Eventos do modal de InventÃ¡rio TOTVS PROTHEUS
  ...
  document.getElementById('btn-download-inv-template').addEventListener('click', downloadProtheusTemplate);

  // BotÃ£o Importar TOTVS Protheus
  const btnImportProtheus = document.getElementById('btn-import-protheus');
  if (btnImportProtheus) {
    btnImportProtheus.addEventListener('click', () => {
      document.getElementById('inventory-protheus-file-input').click();
    });
  }
  const invDropzone = document.getElementById('inventory-dropzone');
  if (invDropzone) {
    invDropzone.addEventListener('click', () => {
      document.getElementById('inventory-protheus-file-input').click();
    });
  }
  const inputProtheus = document.getElementById('inventory-protheus-file-input');
  if (inputProtheus) {
    inputProtheus.addEventListener('change', (e) => {
      if (e.target.files && e.target.files[0]) {
        handleProtheusFileUpload(e.target.files[0]);
      }
    });
  }

  // BotÃ£o Baixar Planilha para Recontagem
  const btnDownloadRecount = document.getElementById('btn-download-recount');
  if (btnDownloadRecount) {
    btnDownloadRecount.addEventListener('click', downloadRecountExcel);
  }

  // BotÃ£o Enviar Segunda/Terceira Contagem
  const btnUploadRecount = document.getElementById('btn-upload-recount');
  if (btnUploadRecount) {
    btnUploadRecount.addEventListener('click', () => {
      document.getElementById('inventory-recount-file-input').click();
    });
  }
  const inputRecount = document.getElementById('inventory-recount-file-input');
  if (inputRecount) {
    inputRecount.addEventListener('change', (e) => {
      if (e.target.files && e.target.files[0]) {
        handleRecountFileUpload(e.target.files[0]);
      }
    });
  }

  // BotÃ£o Finalizar InventÃ¡rio
  const btnFinalizeInv = document.getElementById('btn-finalize-inventory');
  if (btnFinalizeInv) {
    btnFinalizeInv.addEventListener('click', finalizeInventory);
  }

  // BotÃ£o Reiniciar / Novo InventÃ¡rio
  const btnResetInv = document.getElementById('btn-reset-inventory');
  if (btnResetInv) {
    btnResetInv.addEventListener('click', resetInventorySession);
  }

  // BotÃ£o Atualizar Vagas do Almoxarifado
  const btnApplyInv = document.getElementById('btn-apply-inventory-adjust');
  if (btnApplyInv) {
    btnApplyInv.addEventListener('click', applyInventoryAdjustments);
  }

  // Filtros da tabela de inventÃ¡rio
  const filterInvSearch = document.getElementById('filter-inventory-search');
  if (filterInvSearch) {
    filterInvSearch.addEventListener('input', renderInventoryUI);
  }
  const filterInvStatus = document.getElementById('filter-inventory-status');
  if (filterInvStatus) {
    filterInvStatus.addEventListener('change', renderInventoryUI);
  }
