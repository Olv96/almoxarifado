...
let inventoryState = {
  ...
  activePhase: 1, // 1 (Confronto Inicial / 1ª Contagem), 2 (2ª contagem), 3 (3ª contagem), 4 (4ª contagem)
  isFinalized: false,
  items: [],
  fileName: '',
  divergentCount: 0,
...
function downloadInventoryTemplate() {
  ...
  // Formato oficial de 4 colunas estritamente solicitado:
  // Coluna A: Codigo
  // Coluna B: Descrição
  // Coluna C: Lote
  // Coluna D: SALDO DO SISTEMA
  ...
  if (typeof XLSX !== 'undefined') {
    ...
    const today = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(wb, `Modelo_Base_Importacao_Saldo_Sistema_${today}.xlsx`);
  } else {
...
function processInventoryAnalysis(rows) {
  ...
  const h0 = String(headerRow[0] || '').toLowerCase().trim();
  const h1 = String(headerRow[1] || '').toLowerCase().trim();
  const h2 = String(headerRow[2] || '').toLowerCase().trim();
  const h3 = String(headerRow[3] || '').toLowerCase().trim();

  // Verifica se a primeira linha é cabeçalho
  const hasHeader = h0.includes('cod') || h1.includes('desc') || h2.includes('lot') || h3.includes('saldo') || h3.includes('cont');
  const dataRows = hasHeader ? rows.slice(1) : rows;

  // Detectar se a planilha possui colunas preenchidas de 2ª, 3ª ou 4ª contagens
  let hasCount2 = false; // Coluna E (índice 4)
  let hasCount3 = false; // Coluna F (índice 5)
  let hasCount4 = false; // Coluna G (índice 6)

  dataRows.forEach(r => {
    if (parseCountCell(r[4]) !== null) hasCount2 = true;
    if (parseCountCell(r[5]) !== null) hasCount3 = true;
    if (parseCountCell(r[6]) !== null) hasCount4 = true;
  });

  const isRecountFile = hasCount2 || hasCount3 || hasCount4;

  let incomingPhase = 1;
  if (hasCount4) incomingPhase = 4;
  else if (hasCount3) incomingPhase = 3;
  else if (hasCount2) incomingPhase = 2;

  inventoryState.activePhase = incomingPhase;
  inventoryState.isRecountSheet = isRecountFile;
  ...
  if (!isRecountFile || inventoryState.items.length === 0) {
    // MODO 1: CONFRONTO INICIAL (PLANILHA DE SALDO DO SISTEMA ERP - 4 COLUNAS)
    inventoryState.isFinalized = false;
    const analyzedItems = [];
    ...
    dataRows.forEach(r => {
      ...
      // Coluna D (índice 3): SALDO DO SISTEMA
      const erpQty = parseCountCell(r[3]) ?? 0;
      ...
      if (physicalMap.has(key)) {
        ...
        // Se lote estiver em branco no ERP, buscar itens desse código no estoque físico
        if (!lot) {
          const matches = [];
      ...
      // O saldo já existente no programa conta como a Primeira Contagem
      const c1 = physicalQty;
      let status = 'APPROVED';
    ...
    // Sobras Físicas (itens no Almoxarifado Pro que não constavam no ERP)
    ...
    // MODO 2: RECONTAGEM (O usuário enviou a planilha de recontagem com 2ª ou 3ª contagens preenchidas)
    dataRows.forEach(r => {
      const code = String(r[0] || '').trim();
      const lot = String(r[2] || '').trim();
      if (!code) return;
      ...
      // Localizar o item na lista da sessão de inventário
      const item = inventoryState.items.find(it =>
        it.code.toLowerCase() === cKey && (it.lot || '').toLowerCase() === lKey
      ) || inventoryState.items.find(it => it.code.toLowerCase() === cKey);

      if (item) {
        const val2 = parseCountCell(r[4]);
        const val3 = parseCountCell(r[5]);
        const val4 = parseCountCell(r[6]);

        if (incomingPhase === 2 && val2 !== null) {
          item.c2 = val2;
          // REGRA DA 2ª CONTAGEM:
          // Se for igual ao saldo já existente (c1) OU igual ao saldo da planilha importada (erpQty) -> OK!
          if (item.c2 === item.erpQty) {
            item.status = 'APPROVED';
            item.statusLabel = '🟢 Aprovado (2ª Contagem Bateu com Saldo ERP)';
            item.statusClass = 'row-approved';
            item.needsRecount = false;
            item.finalApprovedQty = item.c2;
          } else if (item.c2 === item.c1) {
            item.status = 'APPROVED';
            item.statusLabel = '🟢 Aprovado (2ª Contagem Bateu com 1ª Contagem)';
            item.statusClass = 'row-approved';
            item.needsRecount = false;
            item.finalApprovedQty = item.c2;
          } else {
            item.status = 'DIVERGENT_2';
            item.statusLabel = '🔴 Divergente na 2ª (Requer 3ª Contagem)';
            item.statusClass = 'row-divergent-2';
            item.needsRecount = true;
            item.finalApprovedQty = null;
          }
        } else if (incomingPhase === 3 && val3 !== null) {
          item.c3 = val3;
          // REGRA DA 3ª CONTAGEM:
          // Se for igual ao ERP OU à 2ª contagem OU à 1ª contagem -> Aprovado!
          if (item.c3 === item.erpQty || item.c3 === item.c2 || item.c3 === item.c1) {
            item.status = 'APPROVED';
            item.statusLabel = '🟢 Aprovado na 3ª Contagem';
            item.statusClass = 'row-approved';
            item.needsRecount = false;
            item.finalApprovedQty = item.c3;
          } else {
            item.status = 'DIVERGENT_3';
            item.statusLabel = '⚠️ Divergência Crítica (Requer 4ª Contagem)';
            item.statusClass = 'row-divergent-3';
            item.needsRecount = true;
            item.finalApprovedQty = null;
          }
        } else if (incomingPhase >= 4 && val4 !== null) {
          item.c4 = val4;
          if (item.c4 === item.erpQty || item.c4 === item.c3 || item.c4 === item.c2 || item.c4 === item.c1) {
            item.status = 'APPROVED';
            item.statusLabel = '🟢 Aprovado na 4ª Contagem';
            item.statusClass = 'row-approved';
            item.needsRecount = false;
            item.finalApprovedQty = item.c4;
          } else {
            item.status = 'DIVERGENT_3';
            item.statusLabel = '⚠️ Divergência Crítica (4ª Contagem Concluída)';
            item.statusClass = 'row-divergent-3';
            item.needsRecount = false;
            item.finalApprovedQty = item.c4;
          }
        ...
        const effectiveFinal = item.finalApprovedQty !== null ? item.finalApprovedQty : (item.c3 || item.c2 || item.c1 || 0);
        item.diff = effectiveFinal - item.erpQty;
      }
    });
  }

  // Recalcular métricas da sessão de inventário
  inventoryState.totalItems = inventoryState.items.length;
  inventoryState.approvedCount = inventoryState.items.filter(it => !it.needsRecount).length;
  inventoryState.divergentCount = inventoryState.items.filter(it => it.needsRecount).length;
  inventoryState.missingCount = inventoryState.items.filter(it => it.status === 'MISSING_PHYSICAL' || it.status === 'SURPLUS_PHYSICAL').length;

  const totalPhysicalUnits = inventoryState.items.reduce((acc, it) => acc + (it.physicalQty || 0), 0);
  const totalERPUnits = inventoryState.items.reduce((acc, it) => acc + (it.erpQty || 0), 0);
...
function renderInventoryResults() {
  ...
  if (phaseLabel) {
    phaseLabel.textContent = inventoryState.divergentCount > 0 && !inventoryState.isFinalized
      ? `Recontagem ${inventoryState.activePhase + 1}ª fase`
      : (inventoryState.isFinalized ? 'Inventário Finalizado' : 'Sem divergências');
  }

  // Atualiza título do arquivo
  if (fileStatusTitle) {
    const phaseName = inventoryState.isFinalized
      ? 'Inventário Finalizado'
      : (inventoryState.isRecountSheet ? `Recontagem Fase ${inventoryState.activePhase}` : 'Confronto Saldo ERP');
  ...
  // Botão de Recontagem: exibido apenas se houver itens divergentes e NÃO estiver finalizado
  if (inventoryState.divergentCount > 0 && !inventoryState.isFinalized) {
    const nextPhase = inventoryState.activePhase + 1;
    recountBtn.classList.remove('hidden');
    recountBtn.innerHTML = `🔄 Baixar Planilha de ${nextPhase}ª Contagem (<span id="recount-items-count">${inventoryState.divergentCount}</span>)`;
  } else {
    recountBtn.classList.add('hidden');
  }

  // Botão Finalizar Inventário: visível quando houver itens analisados
  if (finalizeBtn) {
    if (inventoryState.totalItems > 0 && !inventoryState.isFinalized) {
      finalizeBtn.classList.remove('hidden');
      finalizeBtn.textContent = '🏁 Finalizar Inventário';
      finalizeBtn.disabled = false;
    } else if (inventoryState.isFinalized) {
      finalizeBtn.classList.remove('hidden');
      finalizeBtn.textContent = '🎉 Inventário Finalizado';
  ...
  // Botão Aplicar Ajustes: visível apenas quando finalizado ou sem divergências
  if (applyBtn) {
    if (inventoryState.isFinalized || (inventoryState.totalItems > 0 && inventoryState.divergentCount === 0)) {
  ...
  if (theadRow) {
    if (inventoryState.isRecountSheet || inventoryState.isFinalized) {
      let colsHtml = `
        <th>Status</th>
        <th>Código</th>
        <th>Descrição</th>
        ...
        <th>2ª Contagem</th>
      `;
      if (inventoryState.activePhase >= 3 || inventoryState.isFinalized) {
  ...
  tbody.innerHTML = '';
  if (filtered.length === 0) {
    tbody.innerHTML = `<tr><td colspan="11" class="text-muted" style="text-align: center; padding: 2rem;">Nenhum item encontrado com o filtro selecionado.</td></tr>`;
  } else {
    filtered.forEach(it => {
      const tr = document.createElement('tr');
      ...
      if (inventoryState.isRecountSheet || inventoryState.isFinalized) {
        rowHtml += `
          ...
          <td>${it.c1 !== null && it.c1 !== undefined ? `${it.c1.toLocaleString('pt-BR')} un` : '<span class="text-dim">-</span>'}</td>
          <td>${it.c2 !== null && it.c2 !== undefined ? `${it.c2.toLocaleString('pt-BR')} un` : '<span class="text-dim">-</span>'}</td>
        `;
        if (inventoryState.activePhase >= 3 || inventoryState.isFinalized) {
          rowHtml += `<td>${it.c3 !== null && it.c3 !== undefined ? `${it.c3.toLocaleString('pt-BR')} un` : '<span class="text-dim">-</span>'}</td>`;
        }
        const finalShow = it.finalApprovedQty !== null && it.finalApprovedQty !== undefined ? it.finalApprovedQty : (it.c1 || 0);
        rowHtml += `<td><strong style="color: #38bdf8; font-family: 'JetBrains Mono', monospace;">${finalShow.toLocaleString('pt-BR')} un</strong></td>`;
      } else {
        rowHtml += `
          <td><strong style="color: #cbd5e1; font-family: 'JetBrains Mono', monospace;">${it.erpQty.toLocaleString('pt-BR')} un</strong></td>
...
function downloadRecountExcel() {
  ...
  const phaseToCount = inventoryState.activePhase + 1; // 2 para 2ª contagem, 3 para 3ª contagem, etc.
  let header = [];
  let rows = [];

  if (phaseToCount === 2) {
    // 2ª Contagem: Colunas E e F TOTALMENTE EM BRANCO para contagem em campo
    header = ['Codigo', 'Descricao', 'Lote', 'Primeira contagem', 'Segunda contagem', 'Terceira contagem'];
    rows = recountItems.map(it => [
      it.code,
      it.description,
      it.lot || '',
      it.c1 !== null && it.c1 !== undefined ? it.c1 : 0, // 1ª Contagem preenchida
      '', // 2ª contagem ESTRITAMENTE VAZIA!
      ''  // 3ª contagem ESTRITAMENTE VAZIA!
    ]);
  } else if (phaseToCount === 3) {
    // 3ª Contagem: 1ª e 2ª contagens preenchidas de forma correta, 3ª contagem em branco
    header = ['Codigo', 'Descricao', 'Lote', 'Primeira contagem', 'Segunda contagem', 'Terceira contagem'];
    rows = recountItems.map(it => [
      it.code,
      it.description,
      it.lot || '',
      it.c1 !== null && it.c1 !== undefined ? it.c1 : 0, // 1ª Contagem preenchida
      it.c2 !== null && it.c2 !== undefined ? it.c2 : '', // 2ª Contagem preenchida
      '' // 3ª contagem ESTRITAMENTE VAZIA!
    ]);
  } else {
    // 4ª Contagem em diante
    header = ['Codigo', 'Descricao', 'Lote', 'Primeira contagem', 'Segunda contagem', 'Terceira contagem', 'Quarta contagem'];
    rows = recountItems.map(it => [
      it.code,
      it.description,
      it.lot || '',
      ...
      '' // 4ª contagem ESTRITAMENTE VAZIA!
    ]);
  }
  ...
  if (typeof XLSX !== 'undefined') {
    ...
    XLSX.utils.book_append_sheet(wb, ws, `RECONTAGEM_${phaseToCount}A`);
    const today = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(wb, `Planilha_Recontagem_${phaseToCount}a_Fase_${today}.xlsx`);
  } else {
    alert('Biblioteca de planilhas indisponível.');
  }
}
...
function finalizeInventory() {
  ...
  const confirmMsg = `Deseja realmente finalizar esta operação de inventário?\n\nSerá gerada e baixada a planilha final consolidada contendo todas as contagens registradas durante a operação.`;
  if (!confirm(confirmMsg)) return;

  inventoryState.isFinalized = true;

  // Garante que cada item tenha um saldo final apurado
  inventoryState.items.forEach(it => {
    it.needsRecount = false;
    if (it.finalApprovedQty === undefined || it.finalApprovedQty === null) {
      if (it.c4 !== null && it.c4 !== undefined) it.finalApprovedQty = it.c4;
      else if (it.c3 !== null && it.c3 !== undefined) it.finalApprovedQty = it.c3;
      else if (it.c2 !== null && it.c2 !== undefined) it.finalApprovedQty = it.c2;
      else it.finalApprovedQty = it.c1 !== null && it.c1 !== undefined ? it.c1 : 0;
    }
    it.diff = it.finalApprovedQty - it.erpQty;
    if (it.status && it.status.startsWith('DIVERGENT')) {
      it.statusLabel = '🏁 Finalizado com Auditoria';
    }
  });

  // Exportar planilha final consolidada
  const header = [
    'Codigo',
    'Descricao',
    'Lote',
  ...
  const rows = inventoryState.items.map(it => {
    const finalQty = it.finalApprovedQty;
    const diffFinal = finalQty - it.erpQty;
  ...
  if (typeof XLSX !== 'undefined') {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'INVENTARIO_FINAL');
    const today = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(wb, `Planilha_Final_Inventario_Consolidado_${today}.xlsx`);
  }
  ...
  renderInventoryResults();
  alert('Inventário finalizado com sucesso! A planilha consolidada contendo todas as contagens foi baixada.');
}
...
function exportInventoryAnalysisToExcel() {
  ...
  // Colunas estritamente dispostas para evitar confusão de lote e quantidade:
  // Coluna A: Codigo
  // Coluna B: Descricao
  // Coluna C: Lote
  // Coluna D: Saldo_Sistema_ERP
  // Coluna E: Primeira_Contagem
  // Coluna F: Segunda_Contagem
  // Coluna G: Terceira_Contagem
  // Coluna H: Saldo_Final_Aprovado
  // Coluna I: Diferenca_Final
  // Coluna J: Status_Auditoria
  // Coluna K: Posicoes_Almoxarifado
  const header = [
    'Codigo',
    'Descricao',
    'Lote',
  ...
  const rows = inventoryState.items.map(it => {
    const finalQty = it.finalApprovedQty !== undefined && it.finalApprovedQty !== null
      ? it.finalApprovedQty
      : (it.c1 !== null ? it.c1 : 0);
    const diffFinal = finalQty - it.erpQty;
...
function applyInventoryAdjustments() {
  ...
  const toAdjust = inventoryState.items.filter(it => {
    const finalQty = it.finalApprovedQty !== undefined && it.finalApprovedQty !== null ? it.finalApprovedQty : (it.c1 || 0);
    return finalQty !== it.physicalQty;
  });

  if (toAdjust.length === 0) {
    alert('Todos os itens aprovados já possuem saldo idêntico ao cadastrado no Almoxarifado Pro. Nenhum ajuste necessário!');
    return;
  }
  ...
  toAdjust.forEach(it => {
    const targetQty = it.finalApprovedQty !== undefined && it.finalApprovedQty !== null ? it.finalApprovedQty : (it.c1 || 0);
    let foundAny = false;
        ...
        for (let s = 1; s <= (street.slotsPerLevel || 8); s++) {
          ...
          if (slot && slot.items) {
            ...
            if (foundItem) {
              ...
              foundItem.quantity = Math.max(0, targetQty);
              foundAny = true;
              adjustedCount++;
              ...
              appState.movements.unshift({
                ...
                notes: `Ajuste por inventário: de ${oldQty} para ${foundItem.quantity} un.`
              });
  ...
  alert(`Sucesso! ${adjustedCount} produtos foram ajustados nas vagas do Almoxarifado Pro com auditoria.`);
