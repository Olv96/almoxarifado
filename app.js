...
// Analisador numérico robusto (aceita formatos BR como 1.250,50 ou US como 1250.50 ou inteiros)
...
// Extrai todo o estoque físico consolidado armazenado nas vagas do Almoxarifado Pro
function getOurSystemStockMap() {
  const map = new Map();
  (appState.streets || []).forEach(street => {
    (street.levels || ['B', 'A']).forEach(level => {
      for (let s = 1; s <= (street.slotsPerLevel || 8); s++) {
        const slotCode = `${level}${s}`;
        const slot = getSlot(street.id, slotCode);
        if (!slot || !slot.items) continue;

        slot.items.forEach(it => {
          const parsed = extractProductCodeAndDesc(it.productName);
          const code = (parsed.code || '').trim();
          const desc = (parsed.description || it.productName || '').trim();
          const lot = (it.lot || '').trim();
          const qty = Number(it.quantity) || 0;
          if (!code) return;

          const key = `${code.toLowerCase()}___${lot.toLowerCase()}`;
          const addr = `${street.name} (${slotCode})`;
...
// Baixar modelo TOTVS Protheus (4 colunas)
...
// 1. Processar arquivo do TOTVS PROTHEUS (Comparação: Nosso Sistema x Planilha Importada)
...
function processProtheusComparison(rows, fileName) {
  ...
  let startIndex = 0;
  if (rows.length > 0) {
    const firstRowCol3 = rows[0][3];
    const isNum = typeof firstRowCol3 === 'number' || (firstRowCol3 !== '' && !isNaN(Number(String(firstRowCol3).replace(',', '.'))));
    const firstRowText = (String(rows[0][0]) + ' ' + String(rows[0][1])).toLowerCase();
    if (!isNum || firstRowText.includes('cod') || firstRowText.includes('desc') || firstRowText.includes('quant')) {
      startIndex = 1;
    }
  }
  ...
  const ourStockMap = getOurSystemStockMap();
  const analyzedItems = [];
  ...
  dataRows.forEach(r => {
    ...
    // Coluna D: Quantidade do estoque (Protheus - valor de referência)
    const protheusQty = parseInventoryNumber(r[3]) ?? 0;

    const cKey = code.toLowerCase();
    const lKey = lot.toLowerCase();
    const exactKey = `${cKey}___${lKey}`;

    let systemQty = 0;
    let locations = '(Não consta nas vagas)';
    let isFoundInSystem = false;

    if (ourStockMap.has(exactKey)) {
      const entry = ourStockMap.get(exactKey);
      systemQty = entry.quantity;
      locations = entry.addresses.length > 0 ? entry.addresses.join(', ') : '(Sem endereço)';
      entry.matched = true;
      isFoundInSystem = true;
    } else if (!lot) {
      // Se lote não foi informado no Protheus, busca itens desse código nas vagas
      const matches = [];
      ourStockMap.forEach((val, k) => {
        if (k.startsWith(`${cKey}___`) && !val.matched) {
          matches.push(val);
        }
      });
      if (matches.length > 0) {
        systemQty = matches.reduce((sum, m) => sum + m.quantity, 0);
    ...
    // A quantidade atual do nosso sistema conta como a Primeira Contagem (c1)
    const c1 = systemQty;
    let status = 'APPROVED';
  ...
  // Sobras Físicas: itens presentes no Almoxarifado Pro que não vieram na planilha do Protheus
  ourStockMap.forEach(entry => {
    if (!entry.matched && entry.quantity > 0) {
      analyzedItems.push({
        code: entry.code,
...
function downloadRecountExcel() {
  ...
  if (round === 1) {
    ...
    // Coluna B: Descrição
    // Coluna C: Lotes
    // Coluna D: Primeira contagem (preenchida com c1)
    // Coluna E: Segunda contagem (em branco)
    ...
    rows = recountItems.map(it => [
      ...
      '' // Em branco para a equipe preencher
    ]);
  } else if (round === 2) {
    // 3ª Contagem:
    ...
    rows = recountItems.map(it => [
      ...
      '' // Em branco para desempate
    ]);
  } else {
    // 4ª Contagem em diante
...
function processRecountData(rows) {
  ...
  dataRows.forEach(r => {
    ...
    const item = inventorySession.items.find(it =>
      it.code.toLowerCase() === cKey && (it.lot || '').toLowerCase() === lKey
    ) || inventorySession.items.find(it => it.code.toLowerCase() === cKey);

    if (!item) return;

    if (currentRound === 1) {
      // Estamos recebendo a SEGUNDA CONTAGEM (Coluna E / índice 4)
      const c2 = parseInventoryNumber(r[4]);
      if (c2 !== null) {
        item.c2 = c2;
        recheckedCount++;

        // REGRA SOLICITADA PELO USUÁRIO:
        // "Voce vai comparar a SEGUNDA CONTAGEM com a QUANTIDADE DO SISTEMA e/ou a PRIMEIRA CONTAGEM.."
        if (item.c2 === item.protheusQty) {
      ...
      // Estamos recebendo a TERCEIRA CONTAGEM (Coluna F / índice 5)
      const c3 = parseInventoryNumber(r[5]);
      if (c3 !== null) {
        item.c3 = c3;
        recheckedCount++;

        // Comparar a 3ª contagem com Protheus, com a 2ª ou com a 1ª
        if (item.c3 === item.protheusQty || item.c3 === item.c2 || item.c3 === item.c1) {
          item.status = 'APPROVED';
...
// 4. Finalizar Inventário & Gerar Planilha Consolidada
function finalizeInventory() {
  ...
  let confirmMsg = 'Deseja finalizar esta operação de inventário?\n\nSerá gerada e baixada a planilha final completa com a QUANTIDADE DO TOTVS PROTHEUS e TODAS as contagens realizadas.';
...
// 5. Aplicar Ajustes no Almoxarifado (Opcional - atualiza as vagas físicas com o saldo final aprovado)
function applyInventoryAdjustments() {
  if (!inventorySession.items || inventorySession.items.length === 0) return;

  const toAdjust = inventorySession.items.filter(it => {
    const finalQty = it.approvedQty !== null ? it.approvedQty : it.c1;
    return finalQty !== it.c1;
  });

  if (toAdjust.length === 0) {
    alert('Todos os materiais aprovados já possuem saldo idêntico ao cadastrado no Almoxarifado Pro. Nenhum ajuste necessário!');
    return;
  }

  const confirmMsg = `Confirma a atualização de saldo de ${toAdjust.length} materiais no Almoxarifado Pro com base na auditoria final de inventário?\n\nEsta ação registrará movimentações de CORREÇÃO com o documento "INVENTARIO-TOTVS-${new Date().getFullYear()}".`;
  if (!confirm(confirmMsg)) return;
  ...
  toAdjust.forEach(it => {
    const targetQty = Math.max(0, it.approvedQty !== null ? it.approvedQty : it.c1);
    allStreetsLoop:
          ...
          if (slot && slot.items) {
            ...
            if (foundItem) {
              ...
              appState.movements.unshift({
                ...
                quantity: targetQty,
                lot: foundItem.lot || '',
                docNumber: `INVENTARIO-TOTVS-${new Date().getFullYear()}`,
                notes: `Ajuste por inventário TOTVS Protheus: de ${oldQty} para ${targetQty} un.`
              });
  ...
  alert(`Sucesso! ${adjustedCount} produtos foram atualizados nas vagas do Almoxarifado Pro.`);
  document.getElementById('inventory-modal').close();
}

// 6. Renderizar Interface do Inventário
function renderInventoryUI() {
  ...
  if (!inventorySession.active || inventorySession.items.length === 0) {
    ...
    if (btnDownloadRecount) btnDownloadRecount.classList.add('hidden');
  ...
  const accuracyPct = total > 0 ? Number(((approved / total) * 100).toFixed(1)) : 100.0;

  const totalSysUnits = inventorySession.items.reduce((sum, it) => sum + (it.c1 || 0), 0);
  const totalProtheusUnits = inventorySession.items.reduce((sum, it) => sum + (it.protheusQty || 0), 0);
  const diffUnits = totalSysUnits - totalProtheusUnits;
  const diffUnitsFormatted = diffUnits > 0 ? `+${diffUnits.toLocaleString('pt-BR')}` : diffUnits.toLocaleString('pt-BR');
  ...
  if (pctEl) pctEl.textContent = `${accuracyPct.toFixed(1)}%`;
  if (fillEl) {
  ...
  if (summaryEl) {
    if (inventorySession.isFinalized) {
      summaryEl.textContent = 'Operação de Inventário Finalizada com sucesso! Todas as contagens estão consolidadas.';
    } else if (divergent === 0) {
      summaryEl.textContent = 'Perfeito! 100% dos materiais conferem com o TOTVS Protheus.';
    } else {
      summaryEl.textContent = `${approved} de ${total} materiais conferem perfeitamente. Foram encontradas ${divergent} divergências para recontagem.`;
    }
  }

  if (summaryUnitsEl) {
    summaryUnitsEl.textContent = `Nosso Sistema: ${totalSysUnits.toLocaleString('pt-BR')} un | Protheus: ${totalProtheusUnits.toLocaleString('pt-BR')} un (Diferença: ${diffUnitsFormatted} un)`;
  }

  document.getElementById('inv-kpi-total').textContent = total;
  document.getElementById('inv-kpi-total-units').textContent = `${totalSysUnits.toLocaleString('pt-BR')} un no sistema`;
  document.getElementById('inv-kpi-approved').textContent = approved;
  document.getElementById('inv-kpi-divergent').textContent = divergent;
  document.getElementById('inv-kpi-diff-units').textContent = `${diffUnitsFormatted} un`;

  const phaseLabel = document.getElementById('inv-kpi-phase-label');
  if (phaseLabel) {
    phaseLabel.textContent = divergent > 0 && !inventorySession.isFinalized
      ? (inventorySession.round === 1 ? 'Recontagem 2ª fase' : 'Recontagem 3ª fase')
      : (inventorySession.isFinalized ? 'Inventário Finalizado' : '100% Bateu');
  }
  ...
  if (guidanceBox) {
    if (inventorySession.isFinalized) {
      ...
      guidanceBox.innerHTML = `
        ...
        <p style="margin: 0.25rem 0 0 0;">A planilha consolidada com todos os saldos e contagens foi baixada. Caso deseje atualizar o saldo físico das vagas no Almoxarifado Pro com os saldos finais aprovados, clique em <strong>"Atualizar Vagas do Almoxarifado"</strong> acima.</p>
      `;
    } else if (divergent === 0) {
      guidanceBox.className = 'inventory-guidance-card guidance-success';
      guidanceBox.innerHTML = `
        <strong>🎉 Perfeito! 100% de Acurácia!</strong>
        <p style="margin: 0.25rem 0 0 0;">Todos os materiais do estoque físico conferem exatamente com a planilha do TOTVS Protheus. Clique em <strong>"Finalizar Inventário"</strong> para emitir o relatório consolidado.</p>
      `;
    } else {
      guidanceBox.className = 'inventory-guidance-card guidance-warning';
      const recountLabel = inventorySession.round === 1 ? '2ª Contagem' : `${inventorySession.round + 1}ª Contagem`;
      const uploadBtnLabel = inventorySession.round === 1 ? 'Enviar Segunda Contagem' : `Enviar ${inventorySession.round + 1}ª Contagem`;
      guidanceBox.innerHTML = `
        <strong>⚠️ Atenção: Foram encontradas ${divergent} divergências entre o Nosso Sistema e o TOTVS Protheus.</strong>
        <ol style="margin: 0.35rem 0 0 1.25rem; padding: 0;">
          <li>Clique em <strong>"Baixar Planilha para Recontagem"</strong> para baixar os itens divergentes.</li>
          <li>Conte fisicamente no almoxarifado e preencha a coluna <em>"${recountLabel}"</em>.</li>
          <li>Clique em <strong>"${uploadBtnLabel}"</strong> para enviar a recontagem feita.</li>
        </ol>
      `;
    }
  }

  const recountCountEl = document.getElementById('recount-items-count');
  if (recountCountEl) recountCountEl.textContent = divergent;

  if (divergent > 0 && !inventorySession.isFinalized) {
    if (btnDownloadRecount) {
      btnDownloadRecount.classList.remove('hidden');
      const nextPhaseName = inventorySession.round === 1 ? '2ª Contagem' : `${inventorySession.round + 1}ª Contagem`;
      btnDownloadRecount.innerHTML = `🔄 Baixar Planilha para Recontagem (<span id="recount-items-count">${divergent}</span>)`;
      btnDownloadRecount.title = `Baixar planilha com os ${divergent} itens divergentes para preencher a ${nextPhaseName}`;
    }
    if (btnUploadRecount) {
      btnUploadRecount.classList.remove('hidden');
      const nextPhaseUpload = inventorySession.round === 1 ? 'Enviar Segunda Contagem' : `Enviar ${inventorySession.round + 1}ª Contagem`;
      btnUploadRecount.innerHTML = `📤 ${nextPhaseUpload}`;
    }
  } else {
    if (btnDownloadRecount) btnDownloadRecount.classList.add('hidden');
    if (btnUploadRecount) btnUploadRecount.classList.add('hidden');
  ...
  if (btnFinalize) {
    if (!inventorySession.isFinalized) {
      ...
      btnFinalize.textContent = '🏁 Finalizar Inventário';
      btnFinalize.disabled = false;
    } else {
      btnFinalize.classList.remove('hidden');
      btnFinalize.textContent = '🎉 Inventário Finalizado';
      btnFinalize.disabled = true;
    }
  }

  if (btnApply) {
    if (inventorySession.isFinalized || divergent === 0) {
...
// Configura Drag & Drop na Dropzone de Inventário
