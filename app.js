...
let inventoryState = {
  activePhase: 1, // 1 (1ª contagem), 2 (2ª contagem), 3 (3ª contagem), 4 (4ª contagem...)
  items: [],
  fileName: '',
  divergentCount: 0,
  approvedCount: 0,
...
function downloadInventoryTemplate() {
  ...
  const rows = consolidated.map(it => [
    it.code,
    it.description,
    it.lot || '',
    '', // Primeira contagem (aberta para preenchimento)
    '', // Segunda contagem
    ''  // Terceira contagem
  ]);
  ...
  if (typeof XLSX !== 'undefined') {
    ...
    const today = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(wb, `Planilha_Contagem_Inventario_${today}.xlsx`);
  } else {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (!rows || rows.length <= 1) {
        alert('A planilha não contém dados para contagem.');
        return;
      }
      ...
      alert('Erro ao processar planilha de inventário: ' + err.message);
    }
  };
  reader.readAsArrayBuffer(file);
}
...
function processInventoryAnalysis(rows) {
  ...
  // Mapear estoque atual do sistema consolidado por (code, lot)
  const allStock = getAllStockItems();
  const systemStockMap = new Map();

  allStock.forEach(it => {
    const key = `${(it.code || '').toLowerCase().trim()}___${(it.lot || '').toLowerCase().trim()}`;
    if (!systemStockMap.has(key)) {
      systemStockMap.set(key, {
        code: it.code,
        description: it.description,
        lot: it.lot,
        systemQty: 0,
        addresses: [],
        matchedInSheet: false
      });
    }
    const entry = systemStockMap.get(key);
    entry.systemQty += it.quantity;
    if (!entry.addresses.includes(it.address)) {
  ...
  // Detectar qual a fase ativa da planilha (1ª, 2ª, 3ª ou N-ésima contagem)
  let maxColCountFilled = 3; // 3 = índice da 1ª contagem
  dataRows.forEach(r => {
  ...
  const activePhase = maxColCountFilled - 2; // 3 -> fase 1; 4 -> fase 2; 5 -> fase 3; 6 -> fase 4...
  inventoryState.activePhase = Math.max(1, activePhase);
  ...
  dataRows.forEach(r => {
    ...
    if (!code && !desc) return; // Linha vazia

    const c1 = parseCountCell(r[3]);
    const c2 = parseCountCell(r[4]);
    const c3 = parseCountCell(r[5]);
    const c4 = parseCountCell(r[6]);
    const counts = [c1, c2, c3, c4];

    // Busca no sistema
    const key = `${code.toLowerCase()}___${lot.toLowerCase()}`;
    let systemQty = 0;
    let locations = '(Não consta no sistema)';

    if (systemStockMap.has(key)) {
      const sys = systemStockMap.get(key);
      systemQty = sys.systemQty;
      locations = sys.addresses.join(', ');
      sys.matchedInSheet = true;
    }

    // Regras de Decisão do Inventário:
    let status = 'APPROVED';
    let statusLabel = '🟢 Correto';
    let statusClass = 'row-approved';
    let needsRecount = false;
    let finalCount = 0;

    if (inventoryState.activePhase === 1) {
      if (c1 === null) {
        status = 'DIVERGENT_1';
        statusLabel = '🟠 Não Contado';
        statusClass = 'row-divergent-1';
        needsRecount = true;
        finalCount = 0;
      } else if (c1 === systemQty) {
        status = 'APPROVED';
        statusLabel = '🟢 Correto (1ª Cont.)';
        statusClass = 'row-approved';
        needsRecount = false;
        finalCount = c1;
      } else {
        status = 'DIVERGENT_1';
        statusLabel = '🟠 Divergente (1ª Cont.)';
        statusClass = 'row-divergent-1';
        needsRecount = true;
        finalCount = c1;
      }
    } else if (inventoryState.activePhase === 2) {
      if (c2 !== null) {
        // Regra: Se c2 == systemQty OU c2 == c1 -> Correto!
        if (c2 === systemQty || (c1 !== null && c2 === c1)) {
          status = 'APPROVED';
          statusLabel = '🟢 Correto (Aprovado na 2ª)';
          statusClass = 'row-approved';
          needsRecount = false;
          finalCount = c2;
        } else {
          status = 'DIVERGENT_2';
          statusLabel = '🔴 Divergente (2ª Cont.)';
          statusClass = 'row-divergent-2';
          needsRecount = true;
          finalCount = c2;
        }
      } else {
        if (c1 !== null && c1 === systemQty) {
          status = 'APPROVED';
          statusLabel = '🟢 Correto (1ª Cont.)';
          statusClass = 'row-approved';
          needsRecount = false;
          ...
          status = 'DIVERGENT_2';
          statusLabel = '🔴 Recontagem Pendente';
          statusClass = 'row-divergent-2';
          needsRecount = true;
          finalCount = c1 || 0;
        }
      }
    } else if (inventoryState.activePhase === 3) {
      if (c3 !== null) {
        // Regra: Se c3 == systemQty OU c3 == c2 -> Correto!
        if (c3 === systemQty || (c2 !== null && c3 === c2)) {
          status = 'APPROVED';
          statusLabel = '🟢 Correto (Aprovado na 3ª)';
          statusClass = 'row-approved';
          needsRecount = false;
          finalCount = c3;
        } else {
          status = 'DIVERGENT_3';
          statusLabel = '⚠️ Divergente (3ª Cont.)';
          statusClass = 'row-divergent-3';
          needsRecount = true;
          finalCount = c3;
        }
      } else {
        if (c2 !== null && (c2 === systemQty || (c1 !== null && c2 === c1))) {
          status = 'APPROVED';
          statusLabel = '🟢 Correto (Aprovado na 2ª)';
          statusClass = 'row-approved';
          needsRecount = false;
          finalCount = c2;
        } else if (c1 !== null && c1 === systemQty) {
          status = 'APPROVED';
          statusLabel = '🟢 Correto (1ª Cont.)';
          statusClass = 'row-approved';
          needsRecount = false;
          ...
          status = 'DIVERGENT_3';
          statusLabel = '⚠️ Recontagem Pendente';
          statusClass = 'row-divergent-3';
          needsRecount = true;
          finalCount = c2 || c1 || 0;
        }
      ...
      const curr = counts[inventoryState.activePhase - 1];
      const prev = counts[inventoryState.activePhase - 2];
      if (curr !== null) {
        if (curr === systemQty || (prev !== null && curr === prev)) {
          status = 'APPROVED';
          statusLabel = `🟢 Correto (Aprovado na ${inventoryState.activePhase}ª)`;
          statusClass = 'row-approved';
          needsRecount = false;
          finalCount = curr;
        } else {
          status = 'DIVERGENT_3';
          statusLabel = `⚠️ Divergente (${inventoryState.activePhase}ª Cont.)`;
          statusClass = 'row-divergent-3';
          needsRecount = true;
          finalCount = curr;
        }
    ...
    const diff = finalCount - systemQty;
  ...
  // Regra especial de produto com lote no sistema que não constava na contagem
  systemStockMap.forEach((sysItem) => {
    if (!sysItem.matchedInSheet && sysItem.systemQty > 0) {
      analyzedItems.push({
        code: sysItem.code,
        description: sysItem.description,
        lot: sysItem.lot,
        systemQty: sysItem.systemQty,
        c1: null,
        c2: null,
        c3: null,
        c4: null,
        finalCount: 0,
        diff: -sysItem.systemQty,
        status: 'DIVERGENT_1',
        statusLabel: '🟠 Omitido na Contagem',
        statusClass: 'row-divergent-1',
        needsRecount: true,
        locations: sysItem.addresses.join(', ')
      });
    }
  });
  ...
  const totalPhysicalUnits = analyzedItems.reduce((acc, it) => acc + (it.finalCount || 0), 0);
  inventoryState.totalPhysicalUnits = totalPhysicalUnits;

  inventoryState.accuracyRate = inventoryState.totalItems > 0
    ? ((inventoryState.approvedCount / inventoryState.totalItems) * 100).toFixed(1)
    : 100;
...
function renderInventoryResults() {
  ...
  document.getElementById('inv-kpi-accuracy').textContent = `${inventoryState.accuracyRate}%`;
  ...
  if (fileStatusTitle) {
    fileStatusTitle.innerHTML = `📄 Arquivo carregado: <strong>${escapeHtml(inventoryState.fileName || 'Planilha')}</strong> (Fase: ${inventoryState.activePhase}ª Contagem)`;
  }
  ...
  // Cabeçalho da tabela dinâmico com base na fase ativa
  if (theadRow) {
    let colsHtml = `
      <th>Status</th>
      <th>Código</th>
      ...
      <th>3ª Contagem</th>
    `;
    if (inventoryState.activePhase >= 4) {
      colsHtml += `<th>4ª Contagem</th>`;
    }
    colsHtml += `
      <th>Diferença</th>
      <th>Endereço Sistema</th>
    `;
  ...
  if (filtered.length === 0) {
    ...
    filtered.forEach(it => {
      ...
      if (it.status === 'DIVERGENT_1') badgeClass = 'badge-inv-recount1';
      ...
      let rowHtml = `
        ...
        <td>${escapeHtml(it.description)}</td>
        <td>${it.lot ? `<span class="inv-location-tag">Lt: ${escapeHtml(it.lot)}</span>` : '<span class="text-dim">Sem lote</span>'}</td>
        <td><strong style="color: #cbd5e1; font-family: 'JetBrains Mono', monospace;">${it.systemQty.toLocaleString('pt-BR')} un</strong></td>
        ...
        <td>${it.c3 !== null ? `${it.c3.toLocaleString('pt-BR')} un` : '<span class="text-dim">-</span>'}</td>
      `;

      if (inventoryState.activePhase >= 4) {
        rowHtml += `<td>${it.c4 !== null ? `${it.c4.toLocaleString('pt-BR')} un` : '<span class="text-dim">-</span>'}</td>`;
      }
  ...
  if (footerText) {
    footerText.textContent = `Total: ${inventoryState.totalItems} auditados (${inventoryState.approvedCount} aprovados, ${inventoryState.divergentCount} divergentes). Acuracidade: ${inventoryState.accuracyRate}%`;
  }
}
...
function downloadRecountExcel() {
  ...
  if (recountItems.length === 0) {
    alert('Parabéns! Não existem itens divergentes para recontar. Todos foram aprovados!');
    return;
  }

  const nextPhase = inventoryState.activePhase + 1;
  let header = ['Codigo', 'Descricao', 'Lote', 'Primeira contagem', 'Segunda contagem', 'Terceira contagem'];
  if (nextPhase >= 4) {
    header.push('Quarta contagem');
  }
  ...
  const rows = recountItems.map(it => {
    ...
    if (nextPhase >= 4) {
      row.push(''); // Quarta contagem em branco
    }
    return row;
  });
  ...
  if (typeof XLSX !== 'undefined') {
    ...
    XLSX.utils.book_append_sheet(wb, ws, `RECONTAGEM_${nextPhase}`);
    const today = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(wb, `Planilha_Recontagem_Fase${nextPhase}_${today}.xlsx`);
  }
}
